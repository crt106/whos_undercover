...
- 🎤 麦克风测试功能，进房前可提前检测
- 🗳️ 实时投票淘汰机制
- 🔄 游戏中投票换词（准备阶段半数以上同意即可换词，可重复发起）
- 🕵️ **卧底猜词机制**：房主可设置「每个卧底被淘汰都猜词」或「仅最后一个卧底被淘汰猜词」，猜对平民词即可获胜
...
项目使用 Node.js 内置 `node:test`，无需额外测试框架：

```bash
pnpm test
```

测试直接运行 `server/game.js` 的 `Room` 逻辑，不启动 Express/Socket.IO，也不会调用 webhook。当前覆盖：

- 4 人 1 卧底：重复投票换词、发言、投票淘汰、卧底猜词失败、平民胜利。
- 10 人 3 卧底：多轮发言投票、每次卧底淘汰触发猜词、猜错/超时继续、最终猜对卧底胜利。
- 平票 battle：连续平票会生成 `平票battle-1`、`平票battle-2` 发言 Tab，直到再投票决出淘汰人选。
...
4. 若出现平票，平票玩家进入 `平票battle-N` 额外发言，全体存活玩家仅在平票候选人中再投票，直到决出淘汰者
5. 平民投出所有卧底则胜利；若卧底存活人数 ≥ 平民则卧底胜利
6. **卧底猜词机制**：房主可在准备阶段设置猜词触发方式，默认每个卧底被投票淘汰时都有 30 秒机会猜出平民词语；猜对则卧底获胜，猜错或超时则继续游戏（若已无卧底存活则平民胜，大小写不敏感）
...
| 猜词设置 | 准备阶段，房主可设置「每次淘汰」或「最后卧底」触发卧底猜词，默认「每次淘汰」 |
| 换词 | 准备阶段，半数以上同意可更换词对，可重复发起 |
| 发言 | 轮到自己时描述词语，支持文字或语音，60 秒内完成 |
| 投票 | 发言全部完成后进入投票，30 秒内选出最可疑的人 |
| 平票 battle | 若投票平票，平票候选人按 `平票battle-1`、`平票battle-2` 额外发言，全体玩家再投票直到淘汰一人 |
| 下一轮 | 投票结果展示后房主可手动进入下一轮；若房主无操作或关闭窗口，15 秒后自动进入下一轮 |
